...
def read_raw_random(datapath, file_prefix, separator, data_name, save_dir):
    # here is training and testing split preprocess
    if data_name == "epinion" or data_name == "ciao":
        ...
        rating_df = pd.read_csv(datapath, sep=separator, header=None,
                                names=["userId", "check-in time", "latitude", "longitude", "itemId"], encoding='utf-8')
        rating_df.drop(columns=['check-in time'], inplace=True)
        rating_df.drop(columns=['latitude'], inplace=True)
        rating_df.drop(columns=['longitude'], inplace=True)
    elif "douban" in data_name:
        rating_df = pd.read_csv(datapath, sep=separator, header=None,
                                names=["userId", "itemId", "rating", "timestamp"])
        rating_df.drop(columns=['timestamp'], inplace=True)
        rating_df.drop(columns=['rating'], inplace=True)
    elif data_name == "kuairec":
        rating_df = pd.read_csv(datapath, sep=separator, skiprows=1, header=None,
                                names=["userId", "itemId", "play_duration", "video_duration", "time", "date",
                                       "timestamp", "watch_ratio"])
        rating_df.drop(columns=['play_duration'], inplace=True)
        rating_df.drop(columns=['video_duration'], inplace=True)
        rating_df.drop(columns=['time'], inplace=True)
        rating_df.drop(columns=['date'], inplace=True)
        rating_df.drop(columns=['timestamp'], inplace=True)
        rating_df.drop(columns=['watch_ratio'], inplace=True)
    else:
    ...
    # ml-1m (9,9,9), yelp(8,8,19),  epinion(11, 11, 11), kindle_store(16, 16, 23) / (18,18,18), CDs(9, 9, 15)
    # yelp (13,13,13); epinion (10,10,10) lft, gowalla(15,15,15)
    # douban_movie (10,10,10)
    yelp_num = 15
    while np.min(rdf['user_freq']) <= yelp_num:
        # iteratively remove items and users with less than 10 reviews (ml-1m),
        rdf.drop(rdf.index[rdf['user_freq'] <= yelp_num], inplace=True)
        rdf.reset_index(drop=True, inplace=True)

        rdf['item_freq'] = rdf.groupby('itemId')['itemId'].transform('count')
        rdf.drop(rdf.index[rdf['item_freq'] <= yelp_num], inplace=True)
        rdf.reset_index(drop=True, inplace=True)
    ...
    for u in user_list:
        ...
        if len(vali_like) == 0:
            new_vali_idx = np.random.choice(np.arange(len(train_like)), size=1)[0]
        ...
        if len(test_like) == 0:
            new_test_idx = np.random.choice(np.arange(len(train_like)), size=1)[0]
    ...
    np.save(save_dir+'/user_train_like.npy', np.array(user_train_like, dtype=object))
    np.save(save_dir+'/user_vali_like.npy', np.array(user_vali_like, dtype=object))
    np.save(save_dir+'/user_test_like.npy', np.array(user_test_like, dtype=object))
    ...
    for user, item in test_array:
        test_dict[user].append(int(item))
    for user, item in vali_array:
        vali_dict[user].append(int(item))
...
def preprocess(raw_file, file_prefix, data_name, save_dir, leave_k, min_item_per_user=0, min_user_per_item=0, separator=',', order_by_popularity=True):
    """
    ...
    read_raw_random(raw_file, file_prefix, separator, data_name, save_dir)
