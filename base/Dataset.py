...
class BaseDataset:
    def __init__(self, data_dir, dataset, min_user_per_item=1, min_item_per_user=1, leave_k=5, popularity_order=True):
        ...
        save_dir = os.path.join(self.data_dir, self.data_name)
        epinion_prefix = os.path.join(self.data_dir, self.data_name, "rating")
        ciao_prefix = os.path.join(self.data_dir, self.data_name, "rating")
        gowalla_prefix = os.path.join(self.data_dir, self.data_name, "gowalla")
        douban_prefix = os.path.join(self.data_dir, self.data_name, self.data_name)
        if self.data_name == 'epinion':
        ...
        elif self.data_name == 'ciao':
            self.raw_file = ciao_prefix + '.mat'
            self.file_prefix = self.generate_file_prefix()
            # self.mf_file_prefix = self.generate_file_prefix_mf()
            self.data_file = self.file_prefix + '.data'
            self.info_file = self.file_prefix + '.info'
            # self.MF_info = self.file_prefix + '.pkl'
            self.separator = CONSTANT.DATASET_TO_SEPRATOR[dataset]
        elif "douban" in self.data_name:
            self.raw_file = douban_prefix + '.tsv'
            self.file_prefix = self.generate_file_prefix()
            # self.mf_file_prefix = self.generate_file_prefix_mf()
            self.data_file = self.file_prefix + '.data'
            self.info_file = self.file_prefix + '.info'
            # self.MF_info = self.file_prefix + '.pkl'
            self.separator = CONSTANT.DATASET_TO_SEPRATOR[dataset]
        elif self.data_name == 'gowalla':
            self.raw_file = gowalla_prefix + '.txt.gz'
            self.file_prefix = self.generate_file_prefix()
            # self.mf_file_prefix = self.generate_file_prefix_mf()
            self.data_file = self.file_prefix + '.data'
            self.info_file = self.file_prefix + '.info'
        ...
        if not self.check_dataset_exists():
            ...
            preprocess(self.raw_file, self.file_prefix, self.data_name, save_dir, self.leave_k, self.min_item_per_user, self.min_user_per_item,
    ...
    # def generate_file_prefix_mf(self):
    #     split = 'fixed'
    #     base_directory = os.path.join(self.data_dir, self.data_name, split)
    #     if not os.path.exists(base_directory):
    #         os.makedirs(base_directory)
    ...
    #         os.mkdir(sub_directory)
    #
    #     return os.path.join(sub_directory, 'info')
