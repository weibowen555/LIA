...
class MACR(BaseRecommender):
    def __init__(self, dataset, model_conf, device):
        super(MACR, self).__init__(dataset, model_conf)
    ...
    def forward(self, users, pos_items, neg_items):
        ...
        # users_emb = self.embed_user.weight[users]
        # pos_emb = self.embed_item.weight[pos_items]
        # neg_emb = self.embed_item.weight[neg_items]
        ...
        mf_loss_item = torch.mean(torch.negative(torch.log(torch.sigmoid(self.pos_item_scores) + 1e-10)) + torch.negative(
        ...
        regularization = 0.5 * torch.norm(userEmb0) ** 2 + 0.5 * torch.norm(posEmb0) ** 2 + 0.5 * torch.norm(negEmb0) ** 2
            ...
            if (epoch >= test_from and epoch % test_step == 0) or epoch == num_epochs:
                ...
                if should_stop:
                    ...
                    if updated:
                        ...
                        #                               'mainstream_scores')
                        similarity_file = os.path.join(similarity_dir, 'MACR_scores')
                        if not os.path.exists(similarity_file):
                            os.mkdir(similarity_file)
                        with open(os.path.join(similarity_file, self.time + '_macr_scores.npy'), 'wb') as f:
                            np.save(f, ndcg_test_all)
    ...
    def train_model_per_batch(self, user, pos_items, neg_items):
        ...
        # model forwrad
        mf_loss, reg_loss = self.forward(user, pos_items, neg_items)
