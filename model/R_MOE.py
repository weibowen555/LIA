...
# PCA sklearn
class R_MOE(BaseRecommender):
    # add entropy added to the loss (ce), smaller is better
    def __init__(self, dataset, model_conf, device):
        super(R_MOE, self).__init__(dataset, model_conf)
    ...
    def train_model(self, dataset, evaluator, early_stop, logger, config):
        ...
        u1_file = os.path.join(similarity_dir, 'gate_vals_moe_reg')
        ...
        # self.loss_ = np.zeros((train_matrix.shape[0], self.num_experts))
                ...
                if should_stop:
                    ...
                    if updated:
                        ...
                        with open(os.path.join(u1_path, 'gate_output_dis.npy'), "wb") as f:
                            np.save(f, self.gate_kernels(train_matrix).detach().cpu().numpy())

                        # with open(os.path.join(u2_path, 'loss_output_dis.npy'), "wb") as f:
                        #     np.save(f, self.loss_)
