...
class autoencoder(BaseRecommender):
    def __init__(self, dataset, model_conf, device):
        super(autoencoder, self).__init__(dataset, model_conf)
    ...
    def train_model(self, dataset, evaluator, early_stop, logger, config):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for b, batch_idx in enumerate(batch_loader):
                ...
                # batch_loss = self.train_model_per_batch(batch_matrix, self.item_list[batch_idx])
            ...
            if (epoch >= test_from and epoch % test_step == 0) or epoch == num_epochs:
                ...
                if should_stop:
                    ...
                    if updated:
                        ...
                        #                               'mainstream_scores')
                        similarity_file = os.path.join(similarity_dir, 'autoencoder_scores')
                        if not os.path.exists(similarity_file):
                            os.mkdir(similarity_file)
                        with open(os.path.join(similarity_file, self.time + '_ae_scores.npy'), 'wb') as f:
                            np.save(f, ndcg_test_all)
    ...
    # def train_model_per_batch(self, batch_matrix, item_input, batch_weight=None):
    def train_model_per_batch(self, batch_matrix, batch_weight=None):
        ...
        # loss        
        # loss = self.criterion(output[users, items], batch_matrix[users, items])
    ...
    def get_output(self, dataset):
        test_eval_pos, test_eval_target, _, _ = dataset.test_data()
