...
class BinarizeLayer(nn.Module):
    ...
    def __init__(self, n, input_dim, use_not=False, left=None, right=None):
        ...
        self.disc_num = input_dim
        self.use_not = use_not
        # With signed-weight NOT, BinarizeLayer output is not doubled;
        # negation is encoded in the sign of union layer weights.
        # self.output_dim = self.disc_num + self.n * self.input_dim[1] * 2
        self.output_dim = self.disc_num
    ...
    def get_bound_name(self, feature_name, mean=None, std=None):
        ...
        for i in range(self.input_dim):
            bound_name.append('item ' + str(i)) # item idx
...
class ConjunctionLayer(nn.Module):
    ...
    def binarized_forward(self, x):
        ...
        # Wb = Binarize.apply(self.W - THRESHOLD)
        # # res = x.unsqueeze(-1) * Wb
        # res = Binarize.apply(x).unsqueeze(-1) * Wb
        # Conj_output = torch.amin(res, 1)
        # return Conj_output
...
class LearnableUnionLayer(nn.Module):
    """Union layer where each neuron learns its own operator type (AND/OR)
    via a differentiable gate parameter, instead of a fixed split."""

    def __init__(self, n, input_dim, use_not=False, use_nlaf=False, estimated_grad=False, alpha=0.999, beta=8, gamma=1, gate_init=0.0, init_not=False):
        super(LearnableUnionLayer, self).__init__()
        self.n = n
        self.use_not = use_not
        self.input_dim = input_dim
        self.output_dim = self.n
        ...
        # Single shared weight matrix — sign of W encodes negation (no dimension doubling)
        # W > 0 → positive literal, W < 0 → negated literal (NOT)
        if init_not:
            self.W = nn.Parameter(-torch.ones(input_dim, n))
        else:
            self.W = nn.Parameter(INIT_L + (0.5 - INIT_L) * torch.rand(input_dim, n))

        # Learnable gate per neuron: g > 0 -> conjunction (AND), g <= 0 -> disjunction (OR)
        self.g = nn.Parameter(torch.full((n,), gate_init))
    ...
    def continuous_forward(self, x):
        gate = torch.sigmoid(self.g)  # (n,)

        if self.use_not:
            # Signed-weight NOT: decompose W into positive/negative parts
            W_pos = torch.relu(self.W)      # positive connections
            W_neg = torch.relu(-self.W)     # negative connections (magnitude)

            if self.use_nlaf:
                wl_pos = 1. - 1. / (1. - (W_pos * self.alpha) ** self.beta)
                wl_neg = 1. - 1. / (1. - (W_neg * self.alpha) ** self.beta)
                xl_x = 1. - 1. / (1. - (x * self.alpha) ** self.beta)
                xl_1mx = 1. - 1. / (1. - ((1. - x) * self.alpha) ** self.beta)

                conj_out = 1. / (1. + xl_1mx @ wl_pos + xl_x @ wl_neg) ** self.gamma
                disj_out = 1. - 1. / (1. + xl_x @ wl_pos + xl_1mx @ wl_neg) ** self.gamma
            else:
                conj_out = self.Product.apply(1 - (1 - x).unsqueeze(-1) * W_pos - x.unsqueeze(-1) * W_neg)
                disj_out = 1 - self.Product.apply(1 - x.unsqueeze(-1) * W_pos - (1 - x).unsqueeze(-1) * W_neg)
        else:
            if self.use_nlaf:
                wl = 1. - 1. / (1. - (self.W * self.alpha) ** self.beta)
                xl_conj = 1. - 1. / (1. - ((1. - x) * self.alpha) ** self.beta)
                conj_out = 1. / (1. + xl_conj @ wl) ** self.gamma
                xl_disj = 1. - 1. / (1. - (x * self.alpha) ** self.beta)
                disj_out = 1. - 1. / (1. + xl_disj @ wl) ** self.gamma
            else:
                conj_out = self.Product.apply(1 - (1 - x).unsqueeze(-1) * self.W)
                disj_out = 1 - self.Product.apply(1 - x.unsqueeze(-1) * self.W)

        return gate * conj_out + (1. - gate) * disj_out

    @torch.no_grad()
    def binarized_forward(self, x):
        is_conj = (self.g > 0)  # hard threshold at sigmoid = 0.5

        if self.use_not:
            W_pos_bin = Binarize.apply(self.W - THRESHOLD)     # active positive connections (W > 0.5)
            W_neg_bin = Binarize.apply(-self.W - THRESHOLD)    # active negative connections (W < -0.5)

            # Conjunction fails if any positive literal has x=0 or any negative literal has x=1
            conj_fail = (1 - x) @ W_pos_bin + x @ W_neg_bin
            conj_out = torch.where(conj_fail > 0, torch.zeros_like(conj_fail), torch.ones_like(conj_fail))

            # Disjunction succeeds if any positive literal has x=1 or any negative literal has x=0
            disj_success = x @ W_pos_bin + (1 - x) @ W_neg_bin
            disj_out = torch.where(disj_success > 0, torch.ones_like(disj_success), torch.zeros_like(disj_success))
        else:
            Wb = Binarize.apply(self.W - THRESHOLD)
            conj_res = (1 - x) @ Wb
            conj_out = torch.where(conj_res > 0, torch.zeros_like(conj_res), torch.ones_like(conj_res))
            disj_res = x @ Wb
            disj_out = torch.where(disj_res > 0, torch.ones_like(disj_res), torch.zeros_like(disj_res))

        return torch.where(is_conj, conj_out, disj_out)
    ...
    def edge_count(self):
        Wb = Binarize.apply(torch.abs(self.W) - THRESHOLD)
        return torch.sum(Wb)
    ...
    def get_rules(self, prev_layer, skip_connect_layer):
        is_conj = (self.g.detach() > 0)  # (n,)

        W_data = self.W.t().detach().cpu().numpy()  # (n, input_dim)
        ...
        # First pass: extract raw rule per neuron
        neuron_info = []  # (neuron_idx, rule_tuple or None, is_conjunction)
        for ri, row in enumerate(W_data):
            if self.node_activation_cnt[ri] == 0 or self.node_activation_cnt[ri] == self.forward_tot:
                neuron_info.append((ri, None, None))
                continue

            rule = {}
            for i, w in enumerate(row):
                if abs(w) <= THRESHOLD:
                    continue  # inactive connection
                if merged_dim2id[i][1] == -1:
                    continue

                # Sign of w encodes negation: w > 0 → positive literal, w < 0 → NOT
                not_mul = 1 if w > 0 else -1

                if prev_layer.layer_type == 'binarization' and i >= prev_layer.disc_num:
                    rule[(-1, i)] = 1
                else:
                    rid = merged_dim2id[i]
                    rule[(rid[0] * not_mul, rid[1])] = 1

            rule = tuple(sorted(rule.keys()))
            neuron_info.append((ri, rule, is_conj[ri].item()))

        # Second pass: number AND rules first, then OR rules, to match rule_name ordering
        and_rules = {}
        or_rules = {}
        and_rule_list = []
        or_rule_list = []
        ...
        # AND rules
        and_id = 0
        for ri, rule, is_conjunction in neuron_info:
            if rule is None or not is_conjunction:
                continue
            if rule not in and_rules:
                and_rules[rule] = and_id
                and_rule_list.append(rule)
                and_id += 1
            dim2id[ri] = and_rules[rule]

        # OR rules, shifted by number of AND rules
        or_offset = len(and_rule_list)
        or_id = 0
        for ri, rule, is_conjunction in neuron_info:
            if rule is None or is_conjunction:
                continue
            if rule not in or_rules:
                or_rules[rule] = or_offset + or_id
                or_rule_list.append(rule)
                or_id += 1
            dim2id[ri] = or_rules[rule]

        self.dim2id = dim2id
        self.rule_list = (and_rule_list, or_rule_list)
...
class UnionLayer_AND(nn.Module):
    """The AND layer is used to learn the rule-based representation."""

    def __init__(self, n, input_dim, use_not=False, use_nlaf=False, estimated_grad=False, alpha=0.999, beta=8, gamma=1):
        super(UnionLayer_AND, self).__init__()
        self.n = n
        self.use_not = use_not
        self.input_dim = input_dim
        self.output_dim = self.n
        self.layer_type = 'union_and'
        self.forward_tot = None
        self.node_activation_cnt = None
        self.dim2id = None
        self.rule_list = None
    ...
    def binarized_forward(self, x):
        return self.con_layer.binarized_forward(x)
        # return torch.cat([self.con_layer.binarized_forward(x),
        #                   self.dis_layer.binarized_forward(x)], dim=1)
    ...
    def edge_count(self):
        ...
        # return torch.sum(con_Wb) + torch.sum(dis_Wb)
        con_Wb = Binarize.apply(self.con_layer.W - THRESHOLD)
        return torch.sum(con_Wb)
    
    def l1_norm(self):
        return torch.sum(self.con_layer.W)
    
    def l2_norm(self):
        return torch.sum(self.con_layer.W ** 2)
    ...
    def get_rules(self, prev_layer, skip_connect_layer):
        self.con_layer.forward_tot = self.forward_tot
        self.con_layer.node_activation_cnt = self.node_activation_cnt
        ...
        # shift = max(con_dim2id.values()) + 1
        # dis_dim2id = {k: (-1 if v == -1 else v + shift) for k, v in dis_dim2id.items()}
        dim2id = defaultdict(lambda: -1, {**con_dim2id})
        
        rule_list = (con_rule_list, [])
...
class UnionLayer_OR(nn.Module):
    """The union layer is used to learn the rule-based representation."""

    def __init__(self, n, input_dim, use_not=False, use_nlaf=False, estimated_grad=False, alpha=0.999, beta=8, gamma=1):
        super(UnionLayer_OR, self).__init__()
        self.n = n
        self.use_not = use_not
        self.input_dim = input_dim
        self.output_dim = self.n
        self.layer_type = 'union_or'
        self.forward_tot = None
        self.node_activation_cnt = None
        self.dim2id = None
        self.rule_list = None
    ...
    def binarized_forward(self, x):
        # return torch.cat([self.con_layer.binarized_forward(x),
        #                   self.dis_layer.binarized_forward(x)], dim=1)
        return self.dis_layer.binarized_forward(x)
    
    def edge_count(self):
        # con_Wb = Binarize.apply(self.con_layer.W - THRESHOLD)
        dis_Wb = Binarize.apply(self.dis_layer.W - THRESHOLD)
        return torch.sum(dis_Wb)
    
    def l1_norm(self):
        return torch.sum(self.dis_layer.W)
    
    def l2_norm(self):
        return torch.sum(self.dis_layer.W ** 2)

    def clip(self):
        self.dis_layer.clip()

    def get_rules(self, prev_layer, skip_connect_layer):
        self.dis_layer.forward_tot = self.forward_tot
        self.dis_layer.node_activation_cnt = self.node_activation_cnt

        # get dim2id and rule lists of the conjunction layer and the disjunction layer
        # dim2id: dimension --> (k, rule id)
        # con_dim2id, con_rule_list = extract_rules(prev_layer, skip_connect_layer, self.con_layer)
        dis_dim2id, dis_rule_list = extract_rules(prev_layer, skip_connect_layer, self.dis_layer)

        # shift = max(con_dim2id.values()) + 1
        # dis_dim2id = {k: (-1 if v == -1 else v + shift) for k, v in dis_dim2id.items()}
        dim2id = defaultdict(lambda: -1, {**dis_dim2id})
        
        rule_list = ([], dis_rule_list)
