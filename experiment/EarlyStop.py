        ...
        if self.early_stop_measure == 'all':
            ...
            if self.best_score is None:
                ...
                # if np.mean(list(score.values())[:-1]) > self.best_score_mean:
                    ...
                    # self.best_score_mean = np.mean(list(self.best_score.values())[:-1])
                    not_updated = False
        # elif self.early_stop_measure == 'mean_ndcg':
        #     if self.best_score is None:
        #         self.best_score = [score[key] for key in score if key.startswith('NDCG')]
        #         self.best_epoch = epoch
        #         self.best_score_mean = np.mean(self.best_score)
        #         not_updated = False
        #     else:
        #         not_updated = True
        #         temp_score = [score[key] for key in score if key.startswith('NDCG')]
        #         if np.mean(temp_score) > self.best_score_mean:
        #         # if np.mean(list(score.values())[:-1]) > self.best_score_mean:
